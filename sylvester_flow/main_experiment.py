...
parser.add_argument(
    "--max_beta", type=float, default=1.0, metavar="MB", help="max beta for warm-up"
)
parser.add_argument(
    "--min_beta", type=float, default=0.0, metavar="MB", help="min beta for warm-up"
)
parser.add_argument(
    "-f",
    "--flow",
...
# gpu/cpu
parser.add_argument(
    "--gpu_num", type=int, default=0, metavar="GPU", help="choose GPU to run on."
)
...
def run(args, kwargs):
    ...
    optimizer = optim.Adamax(model.parameters(), lr=args.learning_rate, eps=1.0e-7)
    ...
    plot_training_curve(
        train_loss, val_loss, fname=snap_dir + "/training_curve_%s.pdf" % args.flow
    )
    ...
    print(
        "Average train time per epoch: %.2f +/- %.2f"
    ...
    if args.testing:
        ...
        _, _ = evaluate(
            test_loader, final_model, args, testing=True, file=test_score_file
        )
