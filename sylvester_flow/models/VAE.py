    ...
    def create_encoder(self):
        ...
        if self.input_type == "binary":
            ...
            q_z_var = nn.Sequential(
                ...
                nn.Softplus(),
                nn.Hardtanh(min_val=0.01, max_val=7.0),
            )
            return q_z_nn, q_z_mean, q_z_var
        ...
        if self.input_type == "binary":
            ...
            p_x_nn = nn.Sequential(
                GatedConvTranspose2d(
                    self.z_size, 64, self.last_kernel_size, 1, 0, activation=act
                ),
...
class OrthogonalSylvesterVAE(VAE):
    ...
    def __init__(self, args):
        ...
        self.amor_q = nn.Linear(
            self.q_z_nn_output_dim, self.num_flows * self.z_size * self.num_ortho_vecs
        )
        self.amor_b = nn.Linear(
            self.q_z_nn_output_dim, self.num_flows * self.num_ortho_vecs
        )
...
class HouseholderSylvesterVAE(VAE):
    ...
    def __init__(self, args):
        ...
        self.amor_d = nn.Linear(
            self.q_z_nn_output_dim, self.num_flows * self.z_size * self.z_size
        )
        ...
        self.amor_q = nn.Linear(
            self.q_z_nn_output_dim, self.num_flows * self.z_size * self.num_householder
        )
    ...
    def batch_construct_orthogonal(self, q):
        ...
        vvT = torch.bmm(
            v.unsqueeze(2), v.unsqueeze(1)
        )  # v * v_T : batch_dot( B x L x 1 * B x 1 x L ) = B x L x L

        amat = (
            self._eye - 2 * vvT
        )  # NOTICE: v is already normalized! so there is no need to calculate vvT/vTv
