...
import BISVAE
import SVAE
from BISVAE import bisvae_iw_loss, bisvae_loss_function
from cosine_scheduler import CosineAnnealingLR
from datasets import *
from SVAE import svae_iw_loss, svae_loss_function
from utils import *

parser = argparse.ArgumentParser(description="PyTorch SVAE")
parser.add_argument(
    "--model", type=str, default="SVAE", help="Model: SVAE or BISVAE (default SVAE)"
)
parser.add_argument(
    "--dataset",
    type=str,
...
parser.add_argument(
    "--kvs", type=int, default=0, help="Number of auxiliary variables (default: 0)"
)
parser.add_argument(
    "--tau_dim",
    type=int,
...
parser.add_argument(
    "--batchnorm", type=bool, default=True, help="enables bachnorm training"
)
parser.add_argument(
    "--lr", type=float, default=0.001, help="leanring rate (default: 0.001)"
)
...
parser.add_argument("--z_iw", type=int, default=1, help="importance weight of z")
parser.add_argument("--tau_iw", type=int, default=1, help="importance weight of tau")
parser.add_argument("--outfolder", type=str, help="output folder", default="results")
parser.add_argument("--runid", type=str, help="ID for run", default="testrun")
parser.add_argument(
    "--flowtype",
    type=str,
    default="convflow",
...
parser.add_argument(
    "--flow-layers", type=int, default=0, help="Number of conv flow blocks (default: 0)"
)
parser.add_argument(
    "--kernel", type=int, default=5, help="Kernel width of 1d conv (default: 5)"
)
parser.add_argument(
    "--resume",
    type=int,
...
assert args.dataset in [
    ...
    "f",
    "o",
], "data set must be mnist|fixed|omniglot|cifar10"
...
if args.z_iw > 1:
    optimizer = optim.Adam(
        model.parameters(), lr=args.lr, eps=1e-4
    )  # larger eps for stability in IW loss
else:
...
def train(epoch):
    ...
    annealing = (
        1 if args.anneal_epoch == 0 else min(1.0, 1.0 * epoch / args.anneal_epoch)
    )
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        if args.model == "SVAE":
            ...
            if args.z_iw > 1:
                loss = svae_iw_loss(
                    mu, logvar, z, z_f, logdet, recon_data, data, annealing
                )
            else:
                loss = svae_loss_function(
                    mu, logvar, z, z_f, logdet, recon_data, data, annealing
                )
        ...
        if batch_idx % args.log_interval == 0:
            print(
                "Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}".format(
                    ...
                    100.0 * batch_idx * len(data) / len(train_loader.data),
    ...
    print(
        "====> Epoch: {} Average loss: {:.4f} Bits/Dim: {:.4f} on {}/{} samples.\n ".format(
...
def test(epoch):
    ...
    for data in test_loader:
        ...
        if args.model == "SVAE":
            ...
            test_loss += svae_iw_loss(
                mu, logvar, z, z_f, logdet, recon_data, data
            ).data[0]
        elif args.model == "BISVAE":
            (
                z1_mu,
    ...
    print(
        "====> Test set loss: {:.4f} Bits/Dim: {:.4f}".format(
            test_loss, test_bitsperdim
        )
    )
    return test_loss
...
for epoch in range(args.resume + 1, args.epochs + 1):
    ...
    if epoch % args.test_interval == 0 or epoch == args.epochs:
        ...
        print(
            "====> Best loss:     {:.4f} at epoch {:d}\n".format(best_loss, best_epoch)
        )

        # save model
        torch.save(
            model.state_dict(),
            "%s/%s.model.epoch%d" % (args.outfolder, args.runid, epoch),
        )
