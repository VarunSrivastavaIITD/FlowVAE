...
import os, shutil, re, string, urllib.request, fnmatch
...
def _download_frey_faces(dataset):
    ...
    origin = "http://www.cs.nyu.edu/~roweis/data/frey_rawface.mat"
...
def _download_norb_small(dataset):
    ...
    urllib.request.urlretrieve(
        "http://dl.dropbox.com/u/13294233/smallnorb/smallnorb-"
...
def _download_omniglot(dataset):
    ...
    print(f"downloading data from {origin_eval}")
    urllib.request.urlretrieve(origin_eval, dataset + "/images_evaluation.zip")
    print(f"downloading data from {origin_back}")
    ...
    def _load_image(fn):
        ...
        image = np.abs(image - 255.0) / 255.0
        return image
...
def _download_lwf(dataset, size):
    ...
    cPkl.dump(
        [lfw_people.images.astype("uint8"), lfw_people.target],
        f,
...
def load_imagenet32(
    dataset=_get_datafolder_path() + "/imagenet32", normalize=True, dequantify=True
):
...
def load_frey_faces(
    ...
    if not os.path.isfile(datasetfolder + "/fixed_split.pkl"):
        urllib.request.urlretrieve(
            "https://raw.githubusercontent.com/casperkaae/"
...
def _download_svhn(dataset, extra):
    ...
    print(
        "Downloading data from http://ufldl.stanford.edu/housenumbers/, "
...
def load_20newsgroup(
    ...
    if not os.path.isfile(dataset + "20newsgroup_mf" + str(max_feat) + ".pkl"):
        ...
        train_set, test_set, vocab_train = _bow(
            train_set, test_set, max_features=max_feat
        )
...
def load_rcv1(
    ...
    if not os.path.isfile(dataset + "rcv1_mf" + str(max_features) + ".cpkl"):
        ...
        train_set, test_set, vocab_train = _bow(
            train_set, test_set, max_features=max_features
        )
...
def load_rotten_tomatoes(
    dataset=_get_datafolder_path() + "/rotten_tomatoes/",
    vocab_size=40,
    minimum_len=None,
    maximum_len=None,
    seed=1234,
    ...
    # load review data
    pos = pd.read_csv(fn_pos, delimiter="\t", quoting=3, header=None)
    neg = pd.read_csv(fn_neg, header=None, delimiter="\t", quoting=3)
    pos.columns = ["review"]
    neg.columns = ["review"]
    ...
    if isinstance(vocab_size, int):
        ...
        print(
            "Using VOCAB with the %i most frequent characters \n |%s|"
...
def load_stl10(
    ...
    if normalize is True:
        raise ValueError(
            "Normalization with STL10 loader is not supported. "
            "Create an iterator that normalizes on the fly"
        )
    if dequantify is True:
        raise ValueError(
            "Dequantify is not supported with STL10 loader. "
