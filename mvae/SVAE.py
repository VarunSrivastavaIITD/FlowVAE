...
class Model(nn.Module):
        ...
        for kv in range(self.kvs):
            if kv == 0:
                exec(
                    ...
                    % (kv + 1, self.data_dim, self.noise_dim, self.latent_dim)
                )
            else:
                exec(
                    "self.tau_%d_fc = denselayer(%d+%d, %d)"
                    % (kv + 1, self.latent_dim, self.noise_dim, self.latent_dim)
                )

        # ===============================
        self.fc1 = denselayer(
            self.data_dim + self.latent_dim * self.kvs, self.hidden_dim
        )
    ...
    def forward(self, x, TIW=1):
        ...
        return (
            mu.view(TIW, -1, self.latent_dim),
            logvar.view(TIW, -1, self.latent_dim),
            z.view(TIW, -1, self.latent_dim),
            z_f.view(TIW, -1, self.latent_dim),
            logdet,
            self.decode(z_f).view(TIW, -1, self.data_dim),
        )
...
def _svae_compute_loss_terms(mu, logvar, z, z_f, logdet, recon_x, x, annealing):
    ...
    log_qz_given_x = log_mean_exp(
        log_normal(zs, mus, logvars).sum(-1), dim=0
