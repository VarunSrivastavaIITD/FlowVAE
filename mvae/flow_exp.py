...
def U_z(z, uid=0):
    ...
    if uid == 1:  # Potential 1 in NF paper
        ...
        tmp = torch.cat(
            (
                (-0.5 * ((z2 - w1z) / 0.35) ** 2).view(-1, 1),
                (-0.5 * ((z2 - w1z + w2z) / 0.35) ** 2).view(-1, 1),
            ),
            1,
        )
        ...
        tmp = torch.cat(
            (
                (-0.5 * ((z2 - w1z) / 0.4) ** 2).view(-1, 1),
                (-0.5 * ((z2 - w1z + w3z) / 0.35) ** 2).view(-1, 1),
            ),
            1,
        )
        return -log_sum_exp(tmp, dim=1)
...
if __name__ == "__main__":
    ...
    )  # seems to works much better than ADAM and SGD
    ...
    for step in xrange(int(max_steps)):
        ...
        if flow_type == "svae":
            z0 = torch.ones(
                batch_size, data_dim
            )  # SVAE randomness comes from the last stochastic layer
        else:
    ...
    z1, z2, phat_z = evaluate_bivariate_pdf(
        lambda z: torch.exp(-U_z(z, uid)), range=(-4, 4), npoints=200
    )
    z1_true = z1
    z2_true = z2
    phat_z_true = phat_z
    ...
    z1, z2, q0_z0 = evaluate_bivariate_pdf(
        lambda z: torch.exp(log_stdnormal(z).sum(1, keepdim=True)),
        range=(-4, 4),
        npoints=200,
    )
    ...
    if flow_type == "svae":
        ...
        mu, logvar, z, z_f, logdet, x_dec = model(
            Variable(torch.FloatTensor(z0_), volatile=True).cuda()
        )
        zK_ = z.squeeze()
    else:
    ...
    plt.show()
    plt.savefig(
        "flow_plots/gnf_test_%s_u%d_k%d.pdf" % (flow_type, uid, layers),
        bbox_inches="tight",
    )
    plt.close()
    ...
    plt.show()
    plt.savefig(
        "flow_plots/gnf_test_%s_u%d_k%d_true.eps" % (flow_type, uid, layers),
        bbox_inches="tight",
    )
    plt.close()
    ...
    plt.show()
    plt.savefig(
        "flow_plots/gnf_test_%s_u%d_k%d_learned.eps" % (flow_type, uid, layers),
        bbox_inches="tight",
    )
    plt.close()
