...
class Model(nn.Module):
    def __init__(
        ...
        for kv in range(self.kvs):
            if kv == 0:
                exec(
                    "self.tau_%d_fc = denselayer(%d+%d, %d)"
                    % (kv + 1, self.data_dim, self.noise_dim, self.latent_dim1)
                )
            else:
                exec(
                    "self.tau_%d_fc = denselayer(%d+%d, %d)"
                    % (kv + 1, self.latent_dim1, self.noise_dim, self.latent_dim1)
                )

        # ===============================
        # layers for z1
        self.fc1 = denselayer(
            self.data_dim + self.latent_dim1 * self.kvs, self.hidden_dim1
        )
        ...
        if self.flowtype == "convflow":
            self.flow1 = CNN_FLOW(
                dim=self.latent_dim1,
                cnn_layers=self.flow_layers,
    ...
    def encode_x(self, x, TIW):
        ...
        for kv in range(self.kvs):  # iterate the number of auxiliary variables
            ...
            if self.noise_dim > 0:
                noise = Variable(
                    torch.randn(input.size(0), self.noise_dim), requires_grad=False
                )
                if self.usecuda:
                    noise = noise.cuda()
    ...
    def encode_x_with_zero_noise(self, x, TIW):
        ...
        for kv in range(self.kvs):  # iterate the number of auxiliary variables
            ...
            if self.noise_dim > 0:
                noise = Variable(
                    torch.zeros(input.size(0), self.noise_dim), requires_grad=False
                )
                if self.usecuda:
                    noise = noise.cuda()
...
def _bisvae_compute_loss_terms(
    x,
    z1_mu,
    z1_logvar,
    z1,
    ...
    log_qz1_given_x = log_mean_exp(
        log_normal(z1s, z1_mus, z1_logvars).sum(-1), dim=0
    )  # back to (TIW, bs, latent_dim); this is correct way to compute log q(z|x), importance weights over taus

    log_qz1f_given_x = log_qz1_given_x - logdet1  # iw x BS
    log_qz2f_given_z1f = log_normal(z2, z2_mu, z2_logvar).sum(-1) - logdet2  # iw x BS
    LL = log_px_given_z1f + annealing * (
        log_pz2f + log_pz1f_given_z2f - log_qz1f_given_x - log_qz2f_given_z1f
    )

    return LL


def bisvae_loss_function(
    x,
    z1_mu,
    z1_logvar,
    z1,
