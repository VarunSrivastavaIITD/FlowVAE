...
x_sample = torch.bernoulli(
    torch.from_numpy(train_x[idx[:100], :]).float()
).numpy()  # dynamic
...
if BISVAE_FLAG:
    mu1, logvar1 = model.encode_x_with_zero_noise(
        Variable(torch.from_numpy(x_sample).float().cuda()), 1
    )
