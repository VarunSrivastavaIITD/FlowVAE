...
def log_discretized_logistic(x, mean, logscale, binsize, eps=1e-8):
    ...
    b = y
    logp = (
        -a - F.softplus(-a) - F.softplus(-b) + torch.log(torch.exp(binsize / scale) - 1)
    )
    return logp
...
def log_mean_exp(A, dim):
    ...
    B = (
        torch.log(torch.mean(torch.exp(A - A_max.expand_as(A)), dim=dim, keepdim=True))
        + A_max
    )  # B same shape with A
...
def log_sum_exp(A, dim):
    ...
    B = (
        torch.log(torch.sum(torch.exp(A - A_max.expand_as(A)), dim=dim, keepdim=True))
        + A_max
    )
    B = B.squeeze(dim)
    return B
...
def convert(xs):  # xs: (N * dim)
    ...
    for i in range(0, N):
        for j in range(0, N):
            y[
                i * image_width : (i + 1) * image_width,
                j * image_width : (j + 1) * image_width,
            ] = xs[i * N + j, :].reshape(image_width, image_width)
...
class BatchLoader:
    ...
    def reset(self):
        self.pi = np.arange(self.data.size(0))
