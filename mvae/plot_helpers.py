...
def plotKLs(name, outfolder, xepochs, KL, vmin=0, vmax=2):
    ...
    plt.xlabel("Epochs"), plt.ylabel("#Latent Unit"), plt.title(
        "KL(q|p)"
...
def read_from_log(logfile):
    ...
    return (
        np.asarray(train_loss_x),
        np.asarray(train_loss_y),
        np.asarray(test_loss_x),
        np.asarray(test_loss_y),
    )
...
def plotTSNE(figname, z, target, name, legend=False):
    ...
    if legend:
        lgnd = plt.legend(
            [str(i) for i in range(num_class)],
