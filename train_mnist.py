...
ed_optimizer = optim.Adam(list(encoder.parameters())+list(decoder.parameters()), lr=1e-3)
...
for e in range(num_epochs):
    for batch in train_loader:
        ...
        real_loss = torch.nn.BCEWithLogits()(input=discriminator(real_z), target=torch.ones(batch_size,1).to(device)).sum()
        real_loss.backward()
        fake_z = flow(torch.rand(batch_size,z_dim)).detach()
        fake_loss = torch.nn.BCEWithLogits()(input=discriminator(fake_z), target=torch.zeros(batch_size,1).to(device)).sum()
        fake_loss.backward()
        d_optimizer.step()
        ...
        gen_loss = torch.nn.BCEWithLogits()(input=discriminator(real_z), target=torch.ones(batch_size,1).to(device)).sum()
        gen_loss.backward()
        g_optimizer.step()
    ...
    print("Epoch {} : Disc loss = {:.2e} Gen loss = {:.2e} E-D loss = {:.2e}".format(e,real_loss+fake_loss,gen_loss,ed_loss))    