...
def cross_entropy(
    input, target, weight=None, size_average=True, ignore_index=-100, reduce=True
):
    r"""
    ...
    """
    return nll_loss(
        F.log_softmax(input, 1), target, weight, size_average, ignore_index, reduce
    )
...
def nll_loss(
    ...
    if dim == 2:
        return torch._C._nn.nll_loss(
            input, target, weight, size_average, ignore_index, reduce
        )
    elif dim == 4:
        ...
        if target.size()[1:] != input.size()[2:]:
            raise ValueError(
                "Expected target size {}, got {}".format(out_size, input.size())
            )
...
def calculate_loss(x_mean, x, z_mu, z_var, z_0, z_k, ldj, args, beta=1.0):
    ...
    if args.input_type == "binary":
        loss, rec, kl = binary_loss_function(
            x_mean, x, z_mu, z_var, z_0, z_k, ldj, beta=beta
        )
        bpd = 0.0

    elif args.input_type == "multinomial":
        loss, rec, kl = multinomial_loss_function(
            x_mean, x, z_mu, z_var, z_0, z_k, ldj, args, beta=beta
        )
        bpd = loss.item() / (np.prod(args.input_size) * np.log(2.0))
