...
def sample_gaussian(m, v):
    ...
    # Sample z
    ################################################################################
    normal01 = torch.normal(mean=torch.zeros_like(m),std=torch.ones_like(v))
    z = normal01*torch.sqrt(v) + m
    ################################################################################
...
def log_normal(x, m, v):
    ...
    # the last dimension
    ################################################################################
    log_prob = torch.sum(-(x-m)**2/(2*v) - 0.5*torch.log(2*np.pi*v), dim=-1)
    ################################################################################
    # End of code modification
    ################################################################################
    return log_prob
...
def log_normal_mixture(z, m, v):
    ...
    # in the batch
    ################################################################################
    log_prob_k = log_normal(z.unsqueeze(dim=-2).expand_as(m), m, v)
    log_prob = log_mean_exp(log_prob_k, dim=-1)
    ################################################################################
    # End of code modification
    ################################################################################
    return log_prob
